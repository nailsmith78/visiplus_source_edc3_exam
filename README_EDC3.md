...
# Visiplus - Kanban Node.js Server -  CECI A ETE REALISE A L AIDE DE DIFFERENTES IA, DOCUMENTATION RENDER, MONGO.
  ...
  3.3 Étapes Principales - job - ci_cd.yml
  3.3.1 **Install** : Installation des dépendances et outils nécessaires.
  3.3.2 **Test** : Exécution des tests unitaires, d’intégration et de non-régression.
          ...
          repertoire ./tests
           ...
           C. columns.test.ts
           D. ofs.test.ts
  3.3.3 **docker-build** : Compilation et création des images Docker et push vers DockerHub.
          /!\ ne peut s'executer si Test est False 
  3.3.4 **Déploiement** : Déploiement sur les environnements production via Render + MongoDb Atlas (donc non gérer dans notre cas, le déploiement se fera manuellement sur Render).
     
  4. Extrait du pipeline fichier Ci_Cd.YML du 
  on:
    push:
